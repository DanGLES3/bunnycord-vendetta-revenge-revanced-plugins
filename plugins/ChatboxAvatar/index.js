...
function AvatarAction() {
    ...
    return (<Pressable style={{
        ...
        }} onLongPress={openStatus} onPress={createOpenProfile(self?.id, channel?.id ?? channelId)}>
      <Avatar user={self} guildId={channel?.guild_id} status={status} avatarDecoration={self.avatarDecoration} animate={true} // configurable?
...
function openStatus() {
    ...
    ActionSheet.openLazy(async function () {
...
export const onLoad = () => {
    // add button
    patches.push(after("render", ChatInputNew.default, (args, ret) => {
        const insertPoint = findInReactTree(ret, (x) => x?.children?.[0]?.props?.actions)?.children;
        if (insertPoint)
